function varargout = process_evt_detect_hfos_without_bad(varargin)
...
function sProcess = GetDescription() %#ok<DEFNU>
    sProcess.Comment     = 'Detect HFO all candidates without BAD';
...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>
    ...
    for iFile = 1:numel(sInputs)
        try
            ...
            % ===== NEW: collect all BAD intervals (absolute times) =====
            ...
            for ii = 1:size(F,1)
                ...
                for k = 1:numel(segStart)
                    ...
                    % ===== NEW: reject if overlapping any BAD segment =====
                    evtStart = Time(s1);
                    evtEnd   = Time(s2);
            ...
            if isempty(allStart)
                ...
                bst_report('Info', sProcess, sInputs(iFile), 'No HFO candidates detected outside BAD segments.');
                OutputFiles{iFile} = sInputs(iFile).FileName;
                continue;
            end
            ...
            bst_report('Info', sProcess, sInputs(iFile), sprintf('Detected %d HFO candidates outside BAD segments.', size(evtTimes,2)));
            OutputFiles{iFile} = sInputs(iFile).FileName;
